...
error:
    ...
    {
        LOG("NotifyUserLogon failed with invalid user [login:%s][error code: %u]", 
             pszLoginId ? pszLoginId : "<null>",
             dwError);
    }
    else
    {
        LOG("NotifyUserLogon failed [login:%s][error code: %u]", 
             pszLoginId ? pszLoginId : "<null>",
             dwError);
    }
...
error:
    ...
    if ((dwError == LW_ERROR_NO_SUCH_USER) || (dwError == LW_ERROR_NOT_HANDLED))
    {
        LOG("NotifyUserLogoff failed with invalid user [login:%s][error code:%u]",
             pszLoginId ? pszLoginId : "<null>",
             dwError);
    }
    else
    {
        LOG("NotifyUserLogoff failed [login:%s][error code:%u]",
             pszLoginId ? pszLoginId : "<null>",
             dwError);
    }
