...
// the plug-in sets fResult to a value listed in �Result Codes�.
...
// constants described in the �Constants� section for possible values.
//
// fInAttribInfoOnly
//
...
// data as possible and sets fIOContinueData to a plug-in�defined value that
...
// Discussion
//
// The DirectoryService daemon calls a plug-in�s ProcessRequest entry point
...
// length of the list of records, the plug-in�s routine for processing
// sGetRecordList structures may be called multiple times in order to return
// the complete list. The first time the plug-in�s routine for processing
...
// of records that the plug-in should return even if the plug-in�s routine for
...
// fIOContinueData to a plug-in�defined value that the plug-in can use when the
...
// to zero, set fResult to eDSNoErr, set fIOContinueData to a plug-in�defined
