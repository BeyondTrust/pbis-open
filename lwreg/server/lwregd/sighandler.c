    ...
    )
{
    ...
    if (signal(SIGHUP, RegSrvNOPHandler) < 0) {
        dwError = errno;
...
error:
    ...
    REG_LOG_ERROR("Failed to refresh configuration. [Error code:%d]", dwError);
    ...
    )
{
    ...
    action.sa_handler = RegSrvInterruptHandler;
    ...
    while (bWaitForSignals)
    {
      ...
      REG_LOG_WARNING("Received signal [%d]", which_signal);
