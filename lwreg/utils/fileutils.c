    ...
    )
{
    ...
    dwError = RegMoveFile(pszTmpPath, pszDstPath);
    BAIL_ON_REG_ERROR(dwError);

    bRemoveFile = FALSE;

    dwError = RegChangePermissions(pszDstPath, dwPerms);
    BAIL_ON_REG_ERROR(dwError);
    ...
    )
{
    ...
    dwError = RegGetOwnerAndPermissions(pszSrcPath, &uid, &gid, &mode);
    BAIL_ON_REG_ERROR(dwError);

    dwError = RegCopyFileWithPerms(pszSrcPath, pszDstPath, mode);
    BAIL_ON_REG_ERROR(dwError);

    dwError = RegChangeOwnerAndPermissions(pszDstPath, uid, gid, mode);
    BAIL_ON_REG_ERROR(dwError);
    ...
    )
{
    ...
    dwError = RegCheckFileExists(pszPath, &bExists);
    BAIL_ON_REG_ERROR(dwError);
    ...
    dwError = RegCheckFileExists(szBackupPath, &bExists);
    BAIL_ON_REG_ERROR(dwError);
    ...
    dwError = RegCopyFileWithOriginalPerms(pszPath, szBackupPath);
    BAIL_ON_REG_ERROR(dwError);
    ...
    )
{
    ...
    while (NULL != (pDirEntry = readdir(pDir)))
    {
        ...
        if (S_ISDIR(statbuf.st_mode)) {
            ...
            dwError = RegCreateDirectory(
                            szDstPath,
                            statbuf.st_mode);
            BAIL_ON_REG_ERROR(dwError);

            dwError = RegChangeOwner(
                            szDstPath,
                            ownerUid,
                            ownerGid);
            BAIL_ON_REG_ERROR(dwError);

            dwError = RegCopyDirectory(
                            szSrcPath,
                            ownerUid,
                            ownerGid,
                            szDstPath);
            ...
            dwError = RegCopyFileWithOriginalPerms(
                            szSrcPath,
                            szDstPath);
            BAIL_ON_REG_ERROR(dwError);
            ...
            dwError = RegGetSymlinkTarget(
                            szSrcPath,
                            &pszTargetPath);
            BAIL_ON_REG_ERROR(dwError);

            dwError = RegCreateSymlink(
                            pszTargetPath,
                            szDstPath);
            BAIL_ON_REG_ERROR(dwError);
