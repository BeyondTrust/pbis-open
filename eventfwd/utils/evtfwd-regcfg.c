 ...
 * Abstract:
 *
 *        Likewise Security and Authentication Subsystem (EFDSS)
 *
    ...
    )
{
    ...
    PEFD_CONFIG_REG pReg = NULL;

    dwError = EfdOpenConfig(pszConfigKey, pszPolicyKey, &pReg);
    ...
    )
{
    ...
    dwError = EfdAllocateMemory(sizeof(EFD_CONFIG_REG), (PVOID*)&pReg);
    BAIL_ON_EFD_ERROR(dwError);

    dwError = EfdAllocateString(pszConfigKey, &(pReg->pszConfigKey));
    BAIL_ON_EFD_ERROR(dwError);

    dwError = EfdAllocateString(pszPolicyKey, &(pReg->pszPolicyKey));
    BAIL_ON_EFD_ERROR(dwError);
    ...
    )
{
    if ( pReg )
    {
        EFD_SAFE_FREE_STRING(pReg->pszConfigKey);

        EFD_SAFE_FREE_STRING(pReg->pszPolicyKey);
        if ( pReg->hConnection )
        {
            if ( pReg->hKey )
            {
    ...
    )
{
    ...
    if (bGotValue)
    {
        dwError = EfdAllocateString(szValue, &pszValue);
        BAIL_ON_EFD_ERROR(dwError);

        EFD_SAFE_FREE_STRING(*ppszValue);
        *ppszValue = pszValue;
        pszValue = NULL;
    }
