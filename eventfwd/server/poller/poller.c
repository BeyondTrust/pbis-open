...
#define EFD_ILLSEQ_FAIL    1
#define EFD_ILLSEQ_SKIP    1
#define EFD_ILLSEQ_QMARK    1
    ...
    )
{
    ...
    pwszDest = RtlMemoryAllocate(sDestSize);
    if (pwszDest == NULL)
    {
        dwError = ENOMEM;
        BAIL_ON_EFD_ERROR(dwError);
    ...
    )
{
    ...
    for (dwIndex = 0; dwIndex < dwCount; dwIndex++)
    {
        ...
        dwError = EfdSrvWc16sFromMbsWithRepl(
    ...
    )
{
    ...
    {
        EFD_LOG_WARNING("Unable to connect to eventlog");
    }
    if (dwError == rpc_s_connection_closed)
    {
        EFD_LOG_WARNING("Eventlog closed connection");
    }
    BAIL_ON_EFD_ERROR(dwError);
    ...
    EFD_LOG_INFO("%lu events are waiting for the collector",
            dwBacklog);

    if (dwBacklog > 0)
    {
        LwRtlCStringFree(&pszCollector);
        dwError = EfdSrvGetCollectorAddress(NULL, &pszCollector);
        if (dwError == LW_STATUS_NOT_FOUND)
        {
            EFD_LOG_WARNING("The collector address is not set in the config file.");
        ...
        dwError = EfdSrvGetCollectorServicePrincipal(NULL, &pszSPN);
        if (dwError == LW_STATUS_NOT_FOUND)
        {
            dwError = 0;
        }
        ...
        dwError = EfdSrvSetupCredCache(&pAccessToken);
        if (dwError == LWPS_ERROR_INVALID_ACCOUNT)
        {
            EFD_LOG_WARNING("Could not find machine password. The computer is possibly not joined.", pszCollector);
        }
        BAIL_ON_EFD_ERROR(dwError);
        ...
        {
            EFD_LOG_WARNING("The collector [%s] is currently unreachable.", pszCollector);
        }
        if (dwError == LW_STATUS_RESOURCE_NAME_NOT_FOUND)
        {
            EFD_LOG_WARNING("Unable to get service ticket for collector. Check the collector-principal setting.");
        }
        if (dwError == RPC_S_INVALID_NET_ADDR)
        {
            EFD_LOG_WARNING("The collector hostname is not resolvable in DNS");
        }
        BAIL_ON_EFD_ERROR(dwError);
        ...
        EFD_LOG_INFO("Pushing %lu events to collector %s during this period",
                dwPeriodCount,
                pszCollector);
        ...
        dwError = EfdConvertToCollectorRecords(
                        dwPeriodCount,
                        pEventRecords,
                        &pCollectorRecords);
        BAIL_ON_EFD_ERROR(dwError);
        ...
        while (dwBatchIndex < dwPeriodCount && !gbPollerThreadShouldExit)
        {
            ...
            EFD_LOG_INFO("Pushing %lu events to collector in this batch",
                    dwBatchCount);
            ...
            {
                EFD_LOG_ERROR("Insufficient access to write events to the collector.");
            }
            else if (dwError == LW_STATUS_INSUFFICIENT_RESOURCES)
            {
                EFD_LOG_ERROR("Unable to post events to collector. The collector ran out of disk space or ram.");
            }
            BAIL_ON_EFD_ERROR(dwError);
    ...
    )
{
    ...
    EFD_LOG_INFO("Event poller thread started");

    iNextRecordFileFd = open(EFD_NEXT_RECORD_DB, O_RDWR, 0);
    if (iNextRecordFileFd < 0)
    {
        if (errno == ENOENT)
        {
            // The file does not exist, so create it but don't read from it
            iNextRecordFileFd = creat(EFD_NEXT_RECORD_DB, S_IRUSR | S_IWUSR);
            if (iNextRecordFileFd < 0)
            {
                dwError = errno;
                BAIL_ON_EFD_ERROR(dwError);
    ...
    EFD_LOG_INFO("Reading records greater than or equal to %d", dwNextRecordId);
    ...
    while (!gbPollerThreadShouldExit)
    {
        ...
        if (dwError == LW_STATUS_CONNECTION_REFUSED ||//can't contact collector
            ...
            dwError == LWPS_ERROR_INVALID_ACCOUNT || //not joined
        ...
        EFD_LOG_INFO("Event poller sleeping for %f seconds",
    ...
    EFD_LOG_INFO("Event poller thread stopped");
...
cleanup:
    ...
    {
        EFD_LOG_ERROR(
                "Event forwarder polling thread exiting with code %d",
                dwError);
        kill(getpid(), SIGTERM);
    }
    if (pNextRecordFile)
