 ...
 * Abstract:
 *
 *        Event forwarder from eventlogd to collector service Listener
 *
    ...
    )
{
    ...
    dwError = EfdCheckDirectoryExists(pszCachePath, &bDirExists);
    BAIL_ON_EFD_ERROR(dwError);
    ...
    if (!bDirExists)
    {
        ...
        dwError = EfdCreateDirectory(pszCachePath, mode);
        BAIL_ON_EFD_ERROR(dwError);
    }

    dwError = RtlCStringAllocatePrintf(&pszCommPath, "%s/%s",
                                        pszCachePath, EFD_SERVER_FILENAME);
    BAIL_ON_EFD_ERROR(dwError);
