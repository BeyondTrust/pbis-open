...
extern EFD_SERVERINFO  gServerInfo;
extern PEFD_SERVERINFO gpServerInfo;

#define DAEMON_NAME "eventfwdd"
#define PID_DIR     "/var/run"
#define PID_FILE    PID_DIR "/" DAEMON_NAME ".pid"

#define PID_FILE_CONTENTS_SIZE ((9 * 2) + 2)

#define EFD_LOCK_SERVERINFO(bInLock)               \
        if (!bInLock) {                               \
           pthread_mutex_lock(&gpServerInfo->lock);   \
           bInLock = TRUE;                            \
        }

#define EFD_UNLOCK_SERVERINFO(bInLock)             \
        if (bInLock) {                                \
