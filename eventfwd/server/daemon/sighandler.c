    ...
    )
{
    ...
    action.sa_handler = EfdSrvInterruptHandler;
    ...
    while (bWaitForSignals)
    {
        ...
        EFD_LOG_WARNING("Received signal [%d]", which_signal);
        ...
        switch (which_signal)
        {
            ...
            case SIGPIPE:
                {
                    EFD_LOG_DEBUG("Handled SIGPIPE");
                    break;
                }

            case SIGHUP:
                dwError = EfdSrvRefreshConfiguration();
                BAIL_ON_EFD_ERROR(dwError);
                break;
        }
    }
    ...
    )
{
    ...
    EfdSrvSetProcessToExit(TRUE);

    dwError = EfdOpenServer(&hServer);
    BAIL_ON_EFD_ERROR(dwError);
...
cleanup:
    if (hServer != NULL)
    {
        // This could fail since the server is stopped, so the
        // return code is ignored.
        EfdCloseServer(hServer);
    }
    return dwError;
