 ...
 * Abstract:
 *
 *        BeyondTrust SMB Protocol Parser
 *
 *        SMBCheckDirectory
 *
...
{
    NTSTATUS ntStatus = 0;
    PSMB_CHECK_DIRECTORY_REQUEST_HEADER pRequestHeader = NULL;
    PWSTR  pwszPath = NULL;
    PBYTE  pDataCursor = pParams;
    USHORT usByteCountAvailable = 0;
    UCHAR  ucBufferFormat = 0;

    if (ulBytesAvailable < sizeof(SMB_CHECK_DIRECTORY_REQUEST_HEADER))
    {
        ntStatus = STATUS_INVALID_BUFFER_SIZE;
        BAIL_ON_NT_STATUS(ntStatus);
    }

    pRequestHeader = (PSMB_CHECK_DIRECTORY_REQUEST_HEADER)pDataCursor;

    pDataCursor += sizeof(SMB_CHECK_DIRECTORY_REQUEST_HEADER);
    ulBytesAvailable -= sizeof(SMB_CHECK_DIRECTORY_REQUEST_HEADER);
    ulOffset += sizeof(SMB_CHECK_DIRECTORY_REQUEST_HEADER);
...
{
    NTSTATUS ntStatus = 0;
    PSMB_CHECK_DIRECTORY_RESPONSE_HEADER pResponseHeader = NULL;
    USHORT   usPackageBytesUsed = 0;

    if (ulBytesAvailable < sizeof(SMB_CHECK_DIRECTORY_RESPONSE_HEADER))
    {
        ntStatus = STATUS_INVALID_BUFFER_SIZE;
        BAIL_ON_NT_STATUS(ntStatus);
    }

    pResponseHeader = (PSMB_CHECK_DIRECTORY_RESPONSE_HEADER)pParams;
    usPackageBytesUsed += sizeof(SMB_CHECK_DIRECTORY_RESPONSE_HEADER);
