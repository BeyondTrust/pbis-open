...
<xsl:template name="generate-kosek-index">
  ...
  <xsl:variable name="terms" select="//d:indexterm[count(.|key('group-code', i:group-index(normalize-space(concat(d:primary/@sortas, d:primary[not(@sortas) or @sortas = &quot;&quot;]))))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][1]) = 1 and not(@class = 'endofrange')]"/>
  ...
  <div class="index">
    <xsl:apply-templates select="$terms" mode="index-div-kosek">
      ...
      <xsl:sort select="i:group-index(normalize-space(concat(d:primary/@sortas, d:primary[not(@sortas) or @sortas = &quot;&quot;])))" data-type="number"/>
    </xsl:apply-templates>
  </div>
</xsl:template>
...
<xsl:template match="d:indexterm" mode="index-div-kosek">
  ...
  <xsl:variable name="key" select="i:group-index(normalize-space(concat(d:primary/@sortas, d:primary[not(@sortas) or @sortas = &quot;&quot;])))"/>
    ...
    <div class="indexdiv">
      ...
      <dl>
        <xsl:apply-templates select="key('group-code', $key)[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][count(.|key('primary', normalize-space(concat(d:primary/@sortas, d:primary[not(@sortas) or @sortas = &quot;&quot;])))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))][1])=1]" mode="index-primary">
          <xsl:sort select="normalize-space(concat(d:primary/@sortas, d:primary[not(@sortas) or @sortas = &quot;&quot;]))" lang="{$lang}"/>
          <xsl:with-param name="scope" select="$scope"/>
          <xsl:with-param name="role" select="$role"/>
          <xsl:with-param name="type" select="$type"/>
        </xsl:apply-templates>
