...
**  ABSTRACT:
**
**      Callee un/marshalling of pointed_at unsigned long integers
**
**  VERSION: DCE 1.0
**
*/
...
)
{
  ...
  rpc_marshall_ulong_int(mp, (*p_node));
...
rpc_ss_ue_ulong_int
(
    idl_ulong_int **p_referred_to_by,
    rpc_ss_node_type_k_t NIDL_node_type,
    rpc_ss_marsh_state_t *p_unmar_params
)
{
  idl_ulong_int  *p_node = NULL;
  ...
  if ( NIDL_node_type == rpc_ss_unique_node_k )
  {
    ...
    else if (*p_referred_to_by != (idl_ulong_int *)RPC_SS_NEW_UNIQUE_NODE) p_node = *p_referred_to_by;
  }
  ...
  {
    node_size = sizeof(idl_ulong_int );
    if (NIDL_node_type == rpc_ss_mutable_node_k)
    p_node = (idl_ulong_int *)rpc_ss_return_pointer_to_node(
        p_unmar_params->node_table, node_number, node_size,
        NULL, &NIDL_already_unmarshalled, (long *)NULL);
    else
    p_node = (idl_ulong_int *)rpc_ss_mem_alloc(
  ...
  rpc_convert_ulong_int(p_unmar_params->src_drep, ndr_g_local_drep, p_unmar_params->mp, (*p_node));
