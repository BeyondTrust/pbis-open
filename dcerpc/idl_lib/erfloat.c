...
**  ABSTRACT:
**
**      Caller marshalling and unmarshalling of pointed_at float's
**
**  VERSION: DCE 1.0
**
*/
...
rpc_ss_mr_short_float
(
    idl_short_float *p_node,
...
)
{
  ...
  rpc_marshall_short_float(mp, (*p_node));
...
rpc_ss_ur_short_float
(
    idl_short_float **p_referred_to_by,
    rpc_ss_node_type_k_t NIDL_node_type,
    rpc_ss_marsh_state_t *p_unmar_params
)
{
  idl_short_float  *p_node = NULL;
  ...
  if ( NIDL_node_type == rpc_ss_unique_node_k )
  {
    ...
    else if (*p_referred_to_by != (idl_short_float *)RPC_SS_NEW_UNIQUE_NODE) p_node = *p_referred_to_by;
  }
  ...
  if (p_node == NULL)
  {
    node_size =    sizeof(idl_short_float );
    if (NIDL_node_type == rpc_ss_mutable_node_k)
      p_node = (idl_short_float *)rpc_ss_return_pointer_to_node(
          p_unmar_params->node_table, node_number, node_size,
          p_unmar_params->p_allocate, &NIDL_already_unmarshalled, (long *)NULL);
    else
    {
        p_node = (idl_short_float *)rpc_ss_mem_alloc(
  ...
  *p_referred_to_by = p_node;
  ...
  rpc_convert_short_float(p_unmar_params->src_drep, ndr_g_local_drep, p_unmar_params->mp, (*p_node));
