...
**  ABSTRACT:
**
**      Caller marshalling and unmarshalling of pointed_at small integers
**
**  VERSION: DCE 1.0
**
*/
...
)
{
  ...
  mp = NIDL_msp->mp;
  op = NIDL_msp->op;
  rpc_marshall_small_int(mp, (*p_node));
...
)
{
  ...
  if ( NIDL_node_type == rpc_ss_unique_node_k )
  {
    ...
    else if (*p_referred_to_by != (idl_small_int *)RPC_SS_NEW_UNIQUE_NODE) p_node = *p_referred_to_by;
  }
  ...
  if (p_node == NULL)
  {
    node_size =    sizeof(idl_small_int );
    if (NIDL_node_type == rpc_ss_mutable_node_k)
      p_node = (idl_small_int *)rpc_ss_return_pointer_to_node(
          p_unmar_params->node_table, node_number, node_size,
          p_unmar_params->p_allocate, &NIDL_already_unmarshalled, (long *)NULL);
    else
    {
        p_node = (idl_small_int *)rpc_ss_mem_alloc(
  ...
  *p_referred_to_by = p_node;
  ...
  rpc_convert_small_int(p_unmar_params->src_drep, ndr_g_local_drep, p_unmar_params->mp, (*p_node));
