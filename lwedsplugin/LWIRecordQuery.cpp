...
LWIRecordQuery::SetAttributeValues(sSetAttributeValues* pSetAttributeValues)
{
    ...
    while (pAttribute)
    {
        ...
        {
            macError = ADUAdapter_ConvertDSAttributeValuesToMCXValues(pAttribute->pValueListHead, &pMCXValueList);
            GOTO_CLEANUP_ON_MACERROR(macError);
        }
    ...
    macError = ADUAdapter_SaveMCXValuesForGPOSettingType(pMCXValueList, pDirNode->pDirNodeGPO, dwPolicyType, pDirNode->pszDirNodeUserUPN);
    if (macError)
    {
