...
LWIRecordListQuery::Run(IN OUT sGetRecordList* pGetRecordList, LWE_DS_FLAGS Flags, PNETADAPTERINFO pNetAdapterList)
{
    ...
    pGetRecordList->fIOContinueData = (tContextData)0;
...
cleanup:
    ...
    {
        pGetRecordList->fIOContinueData = (tContextData)1;
    }
...
LWIRecordListQuery::ReleaseContinueData(IN OUT sReleaseContinueData* pReleaseContinueData)
{
    ...
    if (pReleaseContinueData->fInContinueData == (tContextData)1)
    {
        /* This is our continue value that we use. Nothing to free. Just return success. */
    }
    else
    {
