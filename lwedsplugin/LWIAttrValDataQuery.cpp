...
LWIAttrValDataQuery::Run(sDoMultiAttrValueSearchWithData* pMultiAttrValueSearchWithData, LWE_DS_FLAGS Flags, PNETADAPTERINFO pNetAdapterList)
{
    ...
    LOG_ENTER("fType = %d, fResult = %d, fInNodeRef = %d, fInAttrType = %s, fInPattMatchType = 0x%04X, fInAttrInfoOnly = %s, fOutDataBuff => { size = %d }",
...
cleanup:
    ...
    LOG_LEAVE("fInOutMatchRecordCount = %d --> %d", pMultiAttrValueSearchWithData->fInOutMatchRecordCount, macError);
