...
long PlugInShell_Initialize(void)
{
    ...
    /* Initialize the macadutil library functions */
    macError = MacADUtilInterface_Initialize();
    GOTO_CLEANUP_ON_MACERROR(macError);

    /* Get the network adpater details - We only care about the ENetAddress info */
    macError = CTGetNetAdapterList(true, &GlobalState.pNetAdapterList);
    GOTO_CLEANUP_ON_MACERROR(macError);
    ...
    macError = ADUAdapter_GetConfigurationSettings(&bMergeModeMCX,
    ...
    if (pszAllowAdministrationBy)
    {
        ...
        macError = ADUAdapter_GetAccessCheckData(GlobalState.pszCurrentAllowedAdminsList, &pAllowAdminCheckData);
        if (macError)
        {
            ...
            LWFreeString(GlobalState.pszCurrentAllowedAdminsList);
    ...
    LOG("Configure LWAuthAdpater");
    macError = LWAuthAdapter::Initialize();
    LOG("LWAuthAdapter setup completed with %d", macError);
    GOTO_CLEANUP_ON_MACERROR(macError);
...
cleanup:
    ...
    {
        LWFreeString(pszUNCProtocolForHomeLocation);
    }
    ...
    {
        ADUAdapter_FreeAccessCheckData(pAllowAdminCheckData);
    }
...
long PlugInShell_PeriodicTask(void)
{
    ...
    if (GlobalState.IsStartupComplete == false)
    {
        ...
        ADUAdapter_GetLsaStatus(&bIsStarted);
        if (bIsStarted)
        {
    ...
    if (bAdminListChanged)
    {
        ...
        {
            ADUAdapter_FreeAccessCheckData(GlobalState.pAllowAdminCheckData);
            GlobalState.pAllowAdminCheckData = NULL;
        }
...
cleanup:
    ...
    if (pszAllowAdministrationBy)
    {
        LWFreeString(pszAllowAdministrationBy);
    }
...
static long Activate(void)
{
    ...
    if (!GlobalState.pszRealm)
    {
        ...
        if (macError)
        {
            LOG("Ignoring error %d from refresh GPO nodes", macError);
            macError = eDSNoErr;
        }
    }
...
static long Deactivate(void)
{
    ...
    if ( GlobalState.pszRealm )
    {
        LWFreeString(GlobalState.pszRealm);
        GlobalState.pszRealm = NULL;
    }
    ...
    if ( GlobalState.pNetAdapterList )
    {
        CTFreeNetAdapterList(GlobalState.pNetAdapterList);
...
static long GetDomainJoinState(PSTR* ppszDomain)
{
    ...
    macError = ADUAdapter_GetADDomain(&pszDomain);
    if (macError)
    {
    ...
    if (pszDomain)
    {
        CTStrToUpper(pszDomain);
    }

    LOG("Joined to domain '%s'", pszDomain ? pszDomain : "");
...
static long RefreshGPONodes(void)
{
    ...
    if (pszDomain)
    {
        ...
        macError = ADUAdapter_EnumWorkgroupManagerEnabledGPOs(pszDomain, &pCurrentGPOs);
