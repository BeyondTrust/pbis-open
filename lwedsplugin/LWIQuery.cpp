...
LWIQuery::LWIQuery(bool bGetValues, long dirNodeRef, LWE_DS_FLAGS Flags, PNETADAPTERINFO pNetAdapterList, PDSDIRNODE pDirNode)
    ...
    OUT LWIQuery** ppQuery)
{
    ...
    LOG_ENTER("bGetValues = %s, fInNodeRef = %u, Flags = 0x%8x ...",
              bGetValues ? "true" : "false",
              dirNodeRef,
              Flags);

    if (Flags == LWE_DS_FLAG_NO_OPTIONS_SET)
        LOG("LWE_DS_FLAG_NO_OPTIONS_SET");

    if (Flags & LWE_DS_FLAG_IS_LEOPARD)
        LOG("LWE_DS_FLAG_IS_LEOPARD");

    if (Flags & LWE_DS_FLAG_IS_SNOW_LEOPARD)
        LOG("LWE_DS_FLAG_IS_SNOW_LEOPARD");

    if (Flags & LWE_DS_FLAG_MERGE_MODE_MCX)
        LOG("LWE_DS_FLAG_MERGE_MODE_MCX");

    if (Flags & LWE_DS_FLAG_USE_AD_UNC_FOR_HOME_LOCATION_SMB)
        LOG("LWE_DS_FLAG_USE_AD_UNC_FOR_HOME_LOCATION_SMB");

    if (Flags & LWE_DS_FLAG_USE_AD_UNC_FOR_HOME_LOCATION_AFP)
        LOG("LWE_DS_FLAG_USE_AD_UNC_FOR_HOME_LOCATION_AFP");

    if (Flags & LWE_DS_FLAG_FORCE_LOCAL_HOME_DIRECTORY_ON_STARTUP_DISK)
        LOG("LWE_DS_FLAG_FORCE_LOCAL_HOME_DIRECTORY_ON_STARTUP_DISK");
    ...
    pQuery = new LWIQuery(bGetValues, dirNodeRef, Flags, pNetAdapterList, pDirNode);
    if (!pQuery)
    {
...
LWIQuery::ShouldQueryComputerGroupInformation()
{
    ...
    return (_recTypeSet &&
            (_dwFlags & LWE_DS_FLAG_IS_LEOPARD) == true &&
    ...
    )
{
    ...
    macError = ADUAdapter_GetUserPrincipalNames(pUser->pw_name, &upn, &userSamAccount, &userDomain);
    GOTO_CLEANUP_ON_MACERROR(macError);
...
cleanup:
    ...
    if (guidString)
    {
        LWFreeString(guidString);
    }

    if (generatedUpn)
    {
        LWFreeString(generatedUpn);
    }
    ...
    if (userSamAccount)
    {
        LWFreeString(userSamAccount);
    }

    if (userDomain)
    {
        LWFreeString(userDomain);
    }
...
LWIQuery::QueryAllGroupInformation(const char* pszName)
{
    ...
    else
    {
        ...
        {
            macError = ADUAdapter_GetSpecificGPO(NULL, _pDirNode->pDirNodeGPO->pszDisplayName, &pGPO);
            GOTO_CLEANUP_ON_MACERROR(macError);
        }
        ...
        if (ADUAdapter_IsMCXSettingEnabledForGPO(pGPO, USER_GROUP_POLICY))
        {
            macError = ADUAdapter_GetMCXValuesForGPOSettingType(pGPO,
                                                                USER_GROUP_POLICY,
                                                                &pMCXValueList);
            GOTO_CLEANUP_ON_MACERROR(macError);
    ...
    )
{
    ...
    if (iStage == 2)
    {
        ...
        macError = ADUAdapter_ConvertMCXSettingsToMCXValues(szPolicyPath, MACHINE_GROUP_POLICY, &pMCXValueList);
        GOTO_CLEANUP_ON_MACERROR(macError);
...
LWIQuery::QueryAllComputerListInformation(const char* pszName)
{
    ...
    else
    {
        ...
        if (ADUAdapter_IsMCXSettingEnabledForGPO(pGPO, MACHINE_GROUP_POLICY))
        {
            macError = ADUAdapter_GetMCXValuesForGPOSettingType(pGPO,
...
cleanup:
    ...
    LWFreeString(pszKeyNameItem1);
    LWFreeString(pszKeyNameItem2);
    ...
    )
{
    ...
    if (Flags & LWE_DS_FLAG_FORCE_LOCAL_HOME_DIRECTORY_ON_STARTUP_DISK)
    {
        macError = ADUAdapter_GetHomeDirectoryDockMCXValue(&pHomeDirDockValue);
        GOTO_CLEANUP_ON_MACERROR(macError);
    }
    ...
    if (iStage == 2)
    {
        ...
        sprintf(szPolicyPath, "%s/user-cache/%ld/%s/%s", LWDS_GPO_CACHE_DIR, (long)uid, szGPOGUID, LWDS_USER_MCX_CSE_GUID);

        macError = ADUAdapter_ConvertMCXSettingsToMCXValues(szPolicyPath, USER_GROUP_POLICY, &pMCXValueList);
        GOTO_CLEANUP_ON_MACERROR(macError);
    }
    ...
    )
{
    ...
    while (fp)
    {
        ...
        macError = ADUAdapter_ConvertMCXSettingsToMCXValues(szPolicyPath, USER_GROUP_POLICY, &pNewList);
        GOTO_CLEANUP_ON_MACERROR(macError);
...
cleanup:
    ...
    LWFreeString(pszHomeDirectoryMount);
    LWFreeString(pszHomeDirectoryXML);
    LWFreeString(pszServer);
    LWFreeString(pszShare);
    ...
    )
{
    ...
    {
        macError = GetUserByName(pszName, &userInfo);
        GOTO_CLEANUP_ON_MACERROR(macError);
    ...
    )
{
    ...
    if (!_userId || _lastUserId != uid)
    {
        ...
        {
            LOG("Call to lwidentity_getpwuid(%d) failed with nssstatus: %d, err: %d", uid, nssStatus, err );
            macError = eDSRecordNotFound;
            GOTO_CLEANUP_ON_MACERROR(macError);
        }
        ...
        macError = ADUAdapter_GetUserADInfo(sUser.pw_uid, &padUserInfo);
        GOTO_CLEANUP_ON_MACERROR(macError);
        ...
        macError = GetGPOGroupMCXSettingsForUser(sUser.pw_name, sUser.pw_uid, &pMCXValueList, Flags);
        GOTO_CLEANUP_ON_MACERROR(macError);

        macError = BuildLWIUser(&sUser,
                                pszUserName,
                                pszNFSHomeDirectory,
                                pszHomeDirectory,
                                pszOriginalNFSHomeDirectory,
...
cleanup:
    ...
    FreeMCXValueList(pMCXValueList);
    ADUAdapter_FreeADUserInfo(padUserInfo);
    LWFreeString(pszNFSHomeDirectory);
    LWFreeString(pszHomeDirectory);
    LWFreeString(pszOriginalNFSHomeDirectory);
    LWFreeString(pszOriginalHomeDirectory);
    ...
    )
{
    ...
    if (!_user || !_lastUsername || strcmp(_lastUsername, pszUsername))
    {
        int err = 0;
        uint32_t nssStatus = NSS_STATUS_SUCCESS;
        char buffer[512+1];
        ...
        if ( nssStatus != NSS_STATUS_SUCCESS || err )
        {
            LOG("Call to lwidentity_getpwnam(%s) failed with nssstatus: %d, err: %d", pszUsername, nssStatus, err );
            macError = eDSRecordNotFound;
            GOTO_CLEANUP_ON_MACERROR( macError );
        }
    ...
    )
{
    ...
    /* Setup our enumeration with auth getpwent routine */
    ...
    )
{
    ...
    if ( nssStatus != NSS_STATUS_SUCCESS || err )
    {
        LOG("Call to lwidentity_getpwent failed with nssstatus: %d, err: %d", nssStatus, err );
        macError = eDSRecordNotFound;
        GOTO_CLEANUP_ON_MACERROR( macError );
    }
    ...
    if (_user)
    {
        if (_user->pw_uid == sUser.pw_uid)
        {
            LOG("Call to lwidentity_getpwent returned duplicate entry (%s %d), stopping enumeration", sUser.pw_name, sUser.pw_uid );
            macError = eDSRecordNotFound;
            GOTO_CLEANUP_ON_MACERROR( macError );
        }
        FreeLWIUser(_user);
    ...
    )
{
    ...
    if (_pDirNode->fPlugInRootConnection)
    {
        if (_lastGroupId != gid)
        {
            ...
            if ( nssStatus != NSS_STATUS_SUCCESS || err )
            {
                LOG("Call to lwidentity_getgrgid failed with nssstatus: %d, err: %d", nssStatus, err );
                macError = eDSRecordNotFound;
                GOTO_CLEANUP_ON_MACERROR(macError);
            }
            ...
            macError = BuildLWIGroup(&grp, NULL, &_groupId);
            GOTO_CLEANUP_ON_MACERROR(macError);
    ...
    )
{
    ...
    PGROUP_POLICY_OBJECT pGPO = NULL;
    PLWIMEMBERLIST pGPOGroupMemberList = NULL;
    ...
    {
        if (strcmp(_lastGroupname, pszName))
        {
            int err = 0;
            uint32_t nssStatus = NSS_STATUS_SUCCESS;
            char buffer[512+1];
            ...
            nssStatus = LWAuthAdapter::getgrnam( pszName, &grp, buffer, buflen, &err );
            if ( nssStatus != NSS_STATUS_SUCCESS || err )
            {
                macError = eDSRecordNotFound;
                goto cleanup;
            ...
            macError = BuildLWIGroup(&grp, NULL, &_group);
            GOTO_CLEANUP_ON_MACERROR(macError);

            strcpy(_lastGroupname, pszName);
            _lastGroupId = -1;
...
cleanup:
    ...
    FreeMCXValueList(pMCXValueList);
    GPA_SAFE_FREE_GPO_LIST(pGPO);
    LWFreeString(pszGPOGUID);
    LWIQuery::FreeMemberList(pGPOGroupMemberList);

    return macError;
}

void
LWIQuery::GetGroupsBegin(
    void
    )
{
    /* Perform class housekeeping */
    ...
    /* Setup our enumeration with auth getgrent routine */
    ...
    )
{
    ...
    if ( nssStatus != NSS_STATUS_SUCCESS || err )
    {
        LOG("Call to lwidentity_getgrent failed with nssstatus: %d, err: %d", nssStatus, err );
        macError = eDSRecordNotFound;
        GOTO_CLEANUP_ON_MACERROR( macError );
    }
    ...
    )
{
    ...
    else if (_pDirNode->fPlugInRootConnection)
    {
        ...
        macError = ADUAdapter_LookupComputerListGPO(pszName, &pszGPOGUID);
        GOTO_CLEANUP_ON_MACERROR(macError);
    ...
    )
{
    ...
    else if (_pDirNode->fPlugInRootConnection)
    {
        ...
        macError = ADUAdapter_LookupComputerGroupGPO(pszName, &pszGPOGUID);
        GOTO_CLEANUP_ON_MACERROR(macError);
...
LWIQuery::SetPasswordChange(PDSRECORD pRecord, const PLWIUSER pUser, bool bSetValue)
{
    ...
    if (pUser && pUser->pw_change)
    {
        if (bSetValue)
        {
            macError = AddAttributeAndValue(kDS1AttrChange,
    ...
    )
{
    ...
    {
        LWFreeString(pAttribute->pszName);
    }
...
LWIQuery::FreeRecord(PDSRECORD pRecord)
{
    ...
    if (pRecord->pszName)
    {
        LWFreeString(pRecord->pszName);
    }

    if (pRecord->pszType)
    {
        LWFreeString(pRecord->pszType);
    }
...
LWIQuery::FreeMemberList(PLWIMEMBERLIST pMemberList)
{
    while(pMemberList)
    {
        ...
        LWFreeString(pCur->pszName);
        LWFreeString(pCur->pszUPN);
        LWFreeMemory(pCur);
    }
}
