...
main (int argc, char* argv[])
{
  ...
  tlog = new TraceLog(root_key, "ttlManager");
  ...
  ProcessInfo::SetHostName(NULL);
	...
	if (0 > srvConf->queryXml(query, values)) {
		logFile = OPENSOAP_TMP_PATH + "log/ttlManager";
	}
	else {
		logFile = values[0] + "ttlManager.log";
	}
	delete srvConf;
  ...
  AppLogger::Write("TTLManger_main start");
#endif
...
//  daemon(0, 1);
  ...
  if (!pidFactory->makePID(TTLMGR_PID_FILE)){
    delete pidFactory;
  ...
  // create instance of TTL Manager and run
  ChannelManager* ttlManagerPtr= new TTLManager();
  try{
    ttlManagerPtr->run();
  }
  catch(...){
  ...
  AppLogger::Write("TTLManager_main wait termination");
  while(ProcessInfo::GetThreadCount()>1){
	  sleep(1);
	  ProcessInfo::CheckThreadInfo();
  }

  delete ttlManagerPtr;
...
#else
  //write stop message
  AppLogger::Write("TTLManger_main stop");
#endif
