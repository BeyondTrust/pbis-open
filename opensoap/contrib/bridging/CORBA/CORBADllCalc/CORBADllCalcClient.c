/*-----------------------------------------------------------------------------
 * $RCSfile: CORBADllCalcClient.c,v $
 *
...
static  const   char    CVS_ID[]        =       "$Id: CORBADllCalcClient.c,v 1.1 2002/03/18 06:21:45 jin Exp $";
#endif  /* _DEBUG */
...
Usage: %s [-s endpoint] [-a soapaction] [-c char_enc] operand_A operator operand_B\n\
  ...
  operand_A      value of operand A\n\
  operator       operator +, -, x, or / \n\
  operand_B      value of operand B\n\
";
...
CalcClientVariablesRelease(CalcClientVariables *app_vars) {
	...
	if (app_vars) {
	  int error_code = 0;
	  fprintf(stderr, "Response Envelope value = %x\n", app_vars->soap_response_env);
		error_code = OpenSOAPEnvelopeRelease(app_vars->soap_response_env);
		ret = 1;
	  ...
	  fprintf(stderr, "Request Envelope value = %x\n", app_vars->soap_request_env);
