    ...
    )
{
    ...
    if (!pInfoFile && errno == ESPIPE)
    {
        // Solaris does not like opening pipes in append mode
        pInfoFile = fopen(pszInfoPath, "w");
    }
    if (!pInfoFile)
    {
        dwError = errno;
    ...
    if (!pErrorFile && errno == ESPIPE)
    {
        // Solaris does not like opening pipes in append mode
        pErrorFile = fopen(pszErrorPath, "w");
    }
    if (!pErrorFile)
    {
        dwError = errno;
