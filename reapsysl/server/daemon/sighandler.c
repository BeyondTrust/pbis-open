    ...
    )
{
    ...
    action.sa_handler = RSysSrvInterruptHandler;
    ...
    while (bWaitForSignals)
    {
      ...
      RSYS_LOG_WARNING("Received signal [%d]", which_signal);
      ...
      switch (which_signal)
        ...
        case SIGPIPE:
          ...
          {
             RSYS_LOG_DEBUG("Handled SIGPIPE");
    ...
    )
{
    ...
    RSysSrvSetProcessToExit(TRUE);

    dwError = RSysOpenServer(&hServer);
    BAIL_ON_RSYS_ERROR(dwError);
