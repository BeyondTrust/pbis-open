...
)
{
    ...
    if (bLeading) {
        RSysStripLeadingWhitespace(pszString);
    }

    if (bTrailing) {
        RSysStripTrailingWhitespace(pszString);
    }
}
    ...
    )
{
    ...
    for (i = 0; i < dwByteArrayLength; i++)
    {
        ...
        dwError = RSysHexCharToByte(hexHi, &ucHi);
        BAIL_ON_RSYS_ERROR(dwError);
      
        dwError = RSysHexCharToByte(hexLow, &ucLow);
        BAIL_ON_RSYS_ERROR(dwError);
...
error:
    ...
    RSYS_SAFE_FREE_MEMORY(pucByteArray);
    *ppucByteArray = NULL;
