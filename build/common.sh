...
function check_is_rhel_21
{
    if [ -f /etc/redhat-release ]; then
        grep "Red Hat Linux .* Server release 2.1" /etc/redhat-release > /dev/null 2>&1
        if [ $? -eq 0 ]; then
            return 0
        fi
    fi
...
function load_profile
{
    ...
    if [ -z "${profile}" ]; then
        warn "Missing build profile"
        exit 1
    fi
    # Shortcut
    if [ "${profile}" = "${BUILD_PROFILE_LOADED}" ]; then
        return 0
    fi
    local profile_file="${BUILD_ROOT}/src/linux/build/profiles/${profile}"
    if [ ! -f "${profile_file}" ]; then
        warn "Missing profile file: $profile_file"
        warn "Bad profile: $profile"
        exit 1
    fi
    # Reset profile variables (so we can nest).
    unset ${BUILD_PRECIOUS_VARIABLES}
    # The profile can add to BUILD_PRECIOUS_VARIABLES to export additional
    # variables.
    export BUILD_PRECIOUS_VARIABLES="BUILD_PRODUCTS BUILD_PLATFORM_PRODUCTS BUILD_SRCDIST BUILD_PREFIX BUILD_SYSCONFDIR BUILD_LOCALSTATEDIR BUILD_VENDOR"
    export BUILD_PROFILE="${profile}"
    source "${profile_file}"
    export ${BUILD_PRECIOUS_VARIABLES}
    export BUILD_PROFILE_LOADED="${BUILD_PROFILE}"
}

# Always load current build profile
load_profile
