    ...
    )
{
    ...
    while (pTemp) {
        ...
        if (dwError == LDAP_REFERRAL) {
            LWUTIL_LOG_INFO("LDAP Directory search resulted in LDAP_REFERRAL error, continuing processing...");
    ...
    )
{
    ...
    if (!pszObjectDN || !*pszObjectDN) {
        return(LWUTIL_ERROR_INVALID_PARAMETER);
    }
    ...
    LWUTIL_LOG_INFO("pszObjectDN: %s pCurrentGroupPolicyObjects=[%.8x]", pszObjectDN, pGroupPolicyObjects);
    ...
    )
{
    ...
    LWUTIL_LOG_INFO("pszObjectDN: %s pszGPOName: %s pGPO=[%.8x]", pszObjectDN, pszGPOName, pGPO);
