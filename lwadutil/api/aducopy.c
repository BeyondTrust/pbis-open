    ...
    )
{
    ...
    do
    {
        ...
        {
            dwError = errno;
            // Can't bail here because that freezes the compiler on OS X 10.6.
    ...
    )
{
    ...
    LWUTIL_LOG_INFO("Purging remote directory %s to replace with context of %s", pszTargetPath, pszSourcePath);
    dwError = ADURemoteRemoveDirRecursive(pszTargetPath); 
    if (dwError) 
    {
        LWUTIL_LOG_WARNING("Failed to remove remote directory %s [dwError:0x%x]", pszTargetPath, dwError);
        dwError = 0;
    }

    dwError = ADURemoteCreateDirectory(pszTargetPath);
    if (dwError)
    {
        LWUTIL_LOG_WARNING("Failed to create remote directory %s [dwError:0x%x]", pszTargetPath, dwError);
        BAIL_ON_LWUTIL_ERROR(dwError);
    }
    ...
    while ((pDirEntry = readdir(pDir)) != NULL)
    {
        ...
        {
            LWUTIL_LOG_INFO("Copying directory %s to %s", pszLocalPath, pszRemotePath);
        ...
        {
            LWUTIL_LOG_INFO("Copying file %s to %s", pszLocalPath, pszRemotePath);
    ...
    )
{
    ...
    {
        LWUTIL_LOG_ERROR("Failed to open/create file [%s][status:0x%x]",
                         pszFileName,
                         status);

        if (status == LW_STATUS_ACCESS_DENIED)
            dwError = LWUTIL_ERROR_ACCESS_DENIED;
        else
            dwError = LWUTIL_ERROR_CREATE_FAILED;
    }
    BAIL_ON_LWUTIL_ERROR(dwError);
    ...
    )
{
    ...
    switch (status)
    {
       ...
       default:
            ...
            LWUTIL_LOG_ERROR("Failed to read from file [status:0x%x]", status);

            dwError = LWUTIL_ERROR_READ_FAILED;
    ...
    )
{
    ...
    if (status != STATUS_SUCCESS)
    {
        LWUTIL_LOG_ERROR("Failed to write to file [status:0x%x]", status);
        dwError = LWUTIL_ERROR_WRITE_FAILED;
    }
    BAIL_ON_LWUTIL_ERROR(dwError);
    ...
    )
{
    ...
    dwError = ADURemoteOpenFile(
                    ...
                    0,                            /* Share access */
                    FILE_OPEN,           /* Create disposition */
                    FILE_DIRECTORY_FILE | FILE_DELETE_ON_CLOSE, /* Create options */
                    &handle);
    BAIL_ON_LWUTIL_ERROR(dwError);
    ...
    for (;;)
    {
                   ...
                   pInfo = (pInfo->NextEntryOffset)?(PFILE_BOTH_DIR_INFORMATION) (((PBYTE) pInfo) + pInfo->NextEntryOffset):NULL)
        {
            ...
            if(pInfo->FileAttributes == FILE_ATTRIBUTE_DIRECTORY)
            {
                ...
                dwError = ADURemoteRemoveDirRecursive(pszRemotePath);
                BAIL_ON_LWUTIL_ERROR(dwError);
                LWUTIL_LOG_INFO("Removed remote sub-directory %s", pszRemotePath);
            }
            else
            {
                ...
                BAIL_ON_LWUTIL_ERROR(dwError);
                LWUTIL_LOG_INFO("Removed remote file %s", pszRemotePath);
            }
        }
    }

    LWUTIL_LOG_INFO("Removed remote directory %s", pszPath);
...
error:
    ...
    LWUTIL_LOG_ERROR("Failed remove directory recursive operation for remote directory %s, [dwError: 0x%x]", pszPath, dwError);
    goto cleanup;
}
