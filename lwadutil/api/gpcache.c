    ...
    )
{
    ...
    if (dwError)
    {
        LWUTIL_LOG_WARNING("GPO GPT.INI file not found on DC, maybe this GPO no longer exists");
        dwError = LWUTIL_ERROR_NO_SUCH_POLICY;
        BAIL_ON_LWUTIL_ERROR(dwError);
    }

    LWUTIL_LOG_INFO("The GPT.INI in sysvol for GPO is at version %d", dwFileGPTVersion);
...
error:
    ...
    if (dwError == LWUTIL_ERROR_NO_SUCH_POLICY)
    {
        dwError = 0;
    }
