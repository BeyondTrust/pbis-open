...
#define BAIL_ON_SEC_ERROR(dwMajorStatus, dwMinorStatus)       \
    if ((dwMajorStatus!= GSS_S_COMPLETE) &&                   \
        ...
        LWUTIL_LOG_ERROR("GSS API Error: Maj:%d Min: %d at %s:%d", dwMajorStatus, dwMinorStatus,  __FILE__, __LINE__); \
        dwError = LWUTIL_ERROR_GSS_API_FAILED;                \
        goto cleanup;                                         \
    }

#define BAIL_ON_KRB_ERROR(ctx, ret)                       \
    if (ret) {                                            \
        LWUTIL_LOG_ERROR("KRB5 error at %s:%d. Error code: %d", __FILE__, __LINE__, ret); \
        switch (ret) {                                    \
        case ENOENT:                                      \
            dwError = ret;                                \
            break;                                        \
        case KRB5_LIBOS_BADPWDMATCH:                      \
            dwError = LWUTIL_ERROR_KRB5_PASSWORD_MISMATCH;\
            break;                                        \
        case KRB5KDC_ERR_KEY_EXP:                         \
            dwError = LWUTIL_ERROR_KRB5_PASSWORD_EXPIRED; \
            break;                                        \
        case KRB5KRB_AP_ERR_SKEW:                         \
            dwError = LWUTIL_ERROR_KRB5_CLOCK_SKEW;       \
            break;                                        \
        default:                                          \
            dwError = LWUTIL_ERROR_KRB5_ERROR;            \
        }                                                 \
        goto error;                                       \
    }
