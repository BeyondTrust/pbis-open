    ...
    )
{
    ...
    MAC_AD_LOG_INFO("Purging remote directory %s to replace with context of %s", pszTargetPath, pszSourcePath);
    dwError = ADURemoteRemoveDirRecursive(pszTargetPath);
    if (dwError)
    {
        MAC_AD_LOG_WARNING("Failed to remove remote directory %s [dwError:0x%x]", pszTargetPath, dwError);
        dwError = 0;
    }
    ...
    if (dwError)
    {
        MAC_AD_LOG_WARNING("Failed to create remote directory %s [dwError:0x%x]", pszTargetPath, dwError);
        BAIL_ON_MAC_ERROR(dwError);
    }
    ...
    while ((pDirEntry = readdir(pDir)) != NULL)
    {
        ...
        {
            MAC_AD_LOG_INFO("Copying directory %s to %s", pszLocalPath, pszRemotePath);
        ...
        {
            MAC_AD_LOG_INFO("Copying file %s to %s", pszLocalPath, pszRemotePath);
    ...
    )
{
    ...
    {
        MAC_AD_LOG_ERROR("Failed to open/create file [%s][status:0x%x]",
                         pszFileName,
                         status);
    ...
    )
{
    ...
    switch (status)
    {
       ...
       default:
            ...
            MAC_AD_LOG_ERROR("Failed to read from file [status:0x%x]", status);
    ...
    )
{
    ...
    if (status != STATUS_SUCCESS)
    {
        MAC_AD_LOG_ERROR("Failed to write to file [status:0x%x]", status);
    ...
    for (;;)
    {
                   ...
                   pInfo = (pInfo->NextEntryOffset)?(PFILE_BOTH_DIR_INFORMATION) (((PBYTE) pInfo) + pInfo->NextEntryOffset):NULL)
        {
            ...
            if(pInfo->FileAttributes == FILE_ATTRIBUTE_DIRECTORY)
            {
                ...
                BAIL_ON_MAC_ERROR(dwError);
                MAC_AD_LOG_INFO("Removed remote sub-directory %s", pszRemotePath);
            }
            else
            {
                ...
                BAIL_ON_MAC_ERROR(dwError);
                MAC_AD_LOG_INFO("Removed remote file %s", pszRemotePath);
            }
        }
    }

    MAC_AD_LOG_INFO("Removed remote directory %s", pszPath);
...
error:
    ...
    MAC_AD_LOG_ERROR("Failed remove directory recursive operation for remote directory %s, [dwError: 0x%x]", pszPath, dwError);
    goto cleanup;
}
