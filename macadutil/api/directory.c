    ...
    )
{
    ...
    if (!ld) {
        MAC_AD_LOG_ERROR("Failed to open LDAP connection to domain controller");
        dwError = errno;
        BAIL_ON_MAC_ERROR(dwError);
        MAC_AD_LOG_ERROR("Failed to get errno for failed open LDAP connection");
    ...
    if (dwError) {
        MAC_AD_LOG_ERROR("Failed to set LDAP option protocol version");
        dwError = errno;
        BAIL_ON_MAC_ERROR(dwError);
        MAC_AD_LOG_ERROR("Failed to get errno for failed set LDAP option");
    ...
    if (dwError) {
        MAC_AD_LOG_ERROR("Failed to set LDAP option to not follow referrals");
        dwError = errno;
        BAIL_ON_MAC_ERROR(dwError);
        MAC_AD_LOG_ERROR("Failed to get errno for failed set LDAP option");
        dwError = MAC_AD_ERROR_LDAP_SET_OPTION;
    ...
    if (dwError) {
        MAC_AD_LOG_ERROR("Failed to set LDAP GSS-API option to allow"
                      " remote principals");
        dwError = errno;
        BAIL_ON_MAC_ERROR(dwError);
        MAC_AD_LOG_ERROR("Failed to get errno for failed set LDAP option");
    ...
    )
{
    ...
    if (dwError) {
        if (dwError == LDAP_NO_SUCH_OBJECT) {
            MAC_AD_LOG_INFO("Caught LDAP_NO_SUCH_OBJECT Error on ldap search");
            goto error;
        }
        if (dwError == LDAP_REFERRAL) {
            MAC_AD_LOG_INFO("Caught LDAP_REFERRAL Error on ldap search");
            MAC_AD_LOG_INFO("LDAP Search Info: DN: [%s]", IsNullOrEmptyString(szObjectDN) ? "<null>" : szObjectDN);
            MAC_AD_LOG_INFO("LDAP Search Info: scope: [%d]", scope);
            MAC_AD_LOG_INFO("LDAP Search Info: query: [%s]", IsNullOrEmptyString(szQuery) ? "<null>" : szQuery);
            if (szAttributeList) {
                size_t i;
                for (i = 0; szAttributeList[i] != NULL; i++) {
                    MAC_AD_LOG_INFO("LDAP Search Info: attribute: [%s]", szAttributeList[i]);
                }
            }
            else {
                MAC_AD_LOG_INFO("Error: LDAP Search Info: no attributes were specified");
            }
        }
        else
        {
            MAC_AD_LOG_ERROR("Error: LDAP Search failed with result: %d", dwError);
            MAC_AD_LOG_INFO("LDAP Search Info: DN: [%s]", IsNullOrEmptyString(szObjectDN) ? "<null>" : szObjectDN);
            MAC_AD_LOG_INFO("LDAP Search Info: scope: [%d]", scope);
            MAC_AD_LOG_INFO("LDAP Search Info: query: [%s]", IsNullOrEmptyString(szQuery) ? "<null>" : szQuery);
            if (szAttributeList) {
    ...
    )
{
    ...
    if (dwError)
    {
        MAC_AD_LOG_ERROR("Failed to update LDAP object attribute string with error: %d, errno: %d", dwError, errno);
    }
    BAIL_ON_MAC_ERROR(dwError);
    ...
    )
{
    ...
    if (dwError)
    {
        MAC_AD_LOG_ERROR("Failed to update LDAP object attribute integer with error: %d, errno: %d", dwError, errno);
    }
    BAIL_ON_MAC_ERROR(dwError);
