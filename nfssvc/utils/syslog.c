...
{
    DWORD dwError = 0;
    PNFSSVC_SYS_LOG pSyslog = NULL;

    dwError = LwAllocateMemory(
                sizeof(NFSSVC_SYS_LOG),
                (PVOID*)&pSyslog);
    if (dwError)
    {
        goto error;
    ...
    NfsSvcSetSyslogMask(maxAllowedLogLevel);
...
{
    PNFSSVC_SYS_LOG pSysLog = (PNFSSVC_SYS_LOG)hLog;
    ...
    )
{
    ...
    NFSSVC_SAFE_FREE_STRING(pSysLog->pszIdentifier);
